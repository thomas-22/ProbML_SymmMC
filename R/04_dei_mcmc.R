...
# 2. Main function
  run_dei_mcmc <- function(
    dataset_names      = NULL,       # e.g. c("linear","sin",…)
    members_count      = 4,          # number of models to use from init_file or ensemble (0 = all)
    init_file          = NULL,       # optional text file listing canonicalized model paths
    ...
    if (is.null(dataset_names)) stop("Please specify dataset_names")
    ...
    ds_files <- list.files(data_path,
                           pattern = paste0("^(",
                                            ...
                                            ")_dataset\\.rds$"),
                           full.names = TRUE)
    if (!length(ds_files)) stop("No datasets found in ", data_path)
    ...
    for (ds in ds_files) {
      ...
      if (!is.null(init_file)) {
        ...
        if (!length(init_list)) stop("Initialization file is empty: ", init_file)
        init_paths <- if (members_count > 0) head(init_list, members_count) else init_list
      } else {
      ...
      estimate_scales <- function(models) {
        weight_list <- purrr::map(models, function(m) unlist(lapply(get_weights(m), as.vector)))
        ...
        list(sigma_W = sigma_W, sigma_b = sigma_b, sigma = sigma)
      }
      ...
      inits_list <- purrr::map(keras_models, function(m) {
        ...
        n_W1 <- H1; n_b1 <- H1; n_W2 <- H2*H1; n_b2 <- H2
        n_W3 <- H3*H2; n_b3 <- H3; n_w4 <- H3; n_b4 <- 1
        ends <- c(n_W1, n_W1+n_b1, n_W1+n_b1+n_W2, n_W1+n_b1+n_W2+n_b2,
                  n_W1+n_b1+n_W2+n_b2+n_W3, n_W1+n_b1+n_W2+n_b2+n_W3+n_b3,
      ...
      out <- file.path(output_path, sprintf("%s_%dchains_draws.rds", ds_name, nchains))
