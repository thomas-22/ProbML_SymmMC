...
\end{aligned}
$$
where each $\sigma$ is a nonlinear activation. @hornik1989multilayer showed that for sufficiently large width this family can approximate any continuous function 
...
$z_{W1flat}$. Remaining layers use non-centered $z$-matrices and vectors with shared log-normal scales and a single noise parameter $\sigma$. Again, Stan’s `reduce_sum` was used.
...
# Results
## Results of DEI-MCMC in the Simulation Study
...
kable(
  ...
  booktabs = TRUE,
  digits   = 4,
  caption  = "Pairwise cosine distances between canonical ensemble centroids"
)
...
\newpage
## Results of DEI-MCMC on the UCI Airfoil dataset
### Ensemble & Canonicalization
...
\begin{figure}[H]
  ...
  \caption{Partial Dependence, Original Initialization NN, Posterior Samples \& Posterior Mean for airfoil\_member01\_canon}
...
\begin{figure}[H]
  ...
  \caption{Partial Dependence, Original Initialization NN, Posterior Samples \& Posterior Mean for airfoil\_member02\_canon}
...
\begin{figure}[H]
  ...
  \caption{Partial Dependence, Original Initialization NN, Posterior Samples \& Posterior Mean for airfoil\_member03\_canon}
...
\begin{figure}[H]
  ...
  \caption{Partial Dependence, Original Initialization NN, Posterior Samples \& Posterior Mean for airfoil\_member04\_canon}
...
# Conclusion
This study set out to test whether a Deep-Ensemble-Initialised MCMC (DEI-MCMC) workflow—including symmetry removal applied after ensembling—can turn fully Bayesian neural-network inference from a theoretical ideal into something that runs overnight on commodity hardware. By first training a handful of deterministic networks, collapsing permutation- and sign-flip symmetries via canonicalization, clustering the resulting weight vectors and then launching one NUTS chain per cluster, we deliberately separated the mode-finding and mode—exploring phases of inference. The abstract promise—efficient posterior exploration without surrendering exactness—has largely been borne out in practice.

In the synthetic sine experiment the four—member ensemble already landed on functionally distinct peaks of the posterior; canonicalization preserved those differences while removing spurious ones. Seeding NUTS at those locations produced healthy bulk-ESS values between ~75 and ~124 and a $\hat R$ of 1.05, indicating good within-mode mixing, yet the aggregated 90 % credibility band remained too tight—evidence that four chains still undersampled the true number of modes.

On the UCI Airfoil Self-Noise data the same machinery broadened the ensemble’s overly confident point forecasts into sensible posterior bands: uncertainty widened precisely where the raw networks disagreed most. At the same time the tougher, higher-dimensional posterior exposed a limitation of the current implementation: with only ~12 h CPU time per chain the bulk-ESS dropped below 41 and the pooled $\hat R$ rose to 2.10, signalling confined exploration within each basi.

Across both cases the symmetry-aware initialisation proved its worth: it removed redundant traversals of equivalent parameter states and gave every Monte-Carlo step a chance to learn something new about the functional landscape of the network. Where the workflow falls short, the bottlenecks are computational rather than conceptual: deeper data sets and richer priors simply demand either more chains, more samples, or faster hardware.
...
First, larger and more diverse ensembles should reduce the risk of missing functionally unique modes. A systematic sweep that grows the ensemble size until additional members cease to enlarge the posterior support—measured, for example, by the clustering distances already used here—would quantify the diminishing returns of more seeds and help formalise the trade-off between number of chains and wall-clock time.

Second, recent findings on mode connectivity suggest that many apparent optima are merely well-separated points on a low-loss manifold (@garipov2018loss; @fort2019deep). Bridging canonicalised modes with curved “connectors” and letting HMC sample along those paths, rather than inside isolated basins, could turn disconnected weight-space islands into a single, traversable continent—potentially cutting the required number of chains even further.
...
Fourth, canonicalization is currently applied once, before sampling. Embedding a dynamic symmetry-resolution step into each leap-frog update could keep chains from drifting back into redundant regions and might improve mixing in the later layers, where over-parameterisation smooths the landscape but also multiplies symmetries.

Fifth, richer diagnostics tailored to multimodal, high-variance posteriors—such as the chain- and layer-wise $\hat R$ measure should accompany any scaling effort to ensure that increased throughput translates into genuinely better uncertainty estimates rather than a cascade of poorly mixed samples.
...
Likewise, @lakshminarayanan2016simple show empirically that beyond three to five independent restarts, further ensemble members yield negligible improvement in uncertainty calibration or predictive performance. Incorporating such mode-stopping criteria into DEI-MCMC—stopping once the ensemble covers a pre-specified fraction of posterior support—could dramatically trim computational cost without sacrificing inferential quality.
...
GitHub Repo for this work: https://github.com/thomas-22/ProbML_SymmMC

![Posterior Predictive Sine Function](../results/Figures/synth_pp_sample_mean_4c.png){#fig-pp_synth_facet fig-cap="Posterior Predictive Sine Function." fig-align="center"}
...
No AI‐generated content was used without rigurous oversight and revision through the author.  
